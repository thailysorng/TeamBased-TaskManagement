...
export const createTask = async (req, res) => {
  try {
    ...
    text =
      text +
      ` The task priority has set a ${priority} priority, so check and act accordingly. The task date is ${new Date(date).toDateString()}. Thank you!!!`;
...
export const getTasks = async (req, res) => {
  try {
    const { userId, isAdmin } = req.user; // get user info from token/session
    const { stage, isTrashed } = req.query;

    // Base query: filter by isTrashed
    let query = { isTrashed: isTrashed ? true : false };

    // If stage is provided, filter by stage
    if (stage) {
      query.stage = stage;
    }
    ...
    // Fetch tasks with populated team info
